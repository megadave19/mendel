...
import type { LogLine, Phase } from '@/components/phase-d/types'
...
export function useMockScan(autoStart = true): MockScanState {
    ...
    SCRIPT.forEach((step) => {
      timers.current.push(
        setTimeout(() => {
          ...
          if (step.issuesFound !== undefined) setIssuesFound(step.issuesFound)
          setLines((prev) => [
            ...prev,
  ...
  return { phase, lines, activeNodeId, elapsedMs, depsScanned, issuesFound, running, done, replay }
}
